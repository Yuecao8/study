...
export default {
  ...
  methods: {
    ...
    // 频道切换
    channelChose () {
      const channelArr = JSON.parse(window.sessionStorage.getItem('channel-chose'));
      const channelActive = JSON.parse(window.sessionStorage.getItem('channel-chose-active'));
      this.channelArr = channelArr || [];
      this.channelActive = channelActive || '0';
      this.changeData();
    },
    // 切换分时数据
    ...
    getParams (param) {
      ...
      if (this.channelArr.length && parseInt(this.channelActive, 10) !== 0) {
        const ids = [];
