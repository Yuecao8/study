var vm = new Vue({
    ...
    methods: {
        ...
        getLunBo() {
            const _this = this;
            param = {
                appkey: "c2ed65c20aea27b9d3f34d2622e523c0",
                appid: "m2ot2aq9fdmjcusxgd",
            }
            url = "http://mapi.0561rtv.cn/api/open/huaibei/shequ_lunbo";
                        ...
                        (res) => {
                            ...
                            if (_this.CityDataList) {
                                res.data.forEach(item => {
                                    if (item.pics_num == 1) {
                                        item.pics[0].filename = _this.imgAdaptation(item.pics[0].filename, '702', '400')
                                    } else if (item.pics_num == 2) {
                                        item.pics.forEach(key => {
                                            key.filename = _this.imgAdaptation(key.filename, '344', '350')
                                        })
                                    } else if (item.pics_num >= 3) {
                                        item.pics.forEach(key => {
                                            key.filename = _this.imgAdaptation(key.filename, '226', '226')
                                        })
                                    }
                                    _this.CityDataList.push(item)
                                });
        ...
        playVideo(id) {
            const _this = this;
            event.preventDefault();
            var audioEleList = _this.$refs.audio;
            audioEleList.forEach(ele=>{
                if(id==ele.getAttribute('data_id')){
                    var nodes = ele.nextElementSibling.childNodes;
                    var img =nodes[2].childNodes[0];
                    if (ele.paused) {
                        ele.play();
                        img.setAttribute('src','./images/yinpin3.gif');
                    } else {
                        ele.pause(); 
                        img.setAttribute('src','./images/wifi.png')
                    }
                    if (ele) {
                        ele.loop = false;
